...
export function GameEconomySettings({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const updateNested = <K extends keyof GameEconomyConfig>(
    key: K,
    field: string,
    value: unknown,
  ) => {
    setEconomy({
      ...config,
      [key]: { ...(config[key] as Record<string, unknown>), [field]: value },
    });
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-2xl border border-white/[0.08] bg-[#0d0d14] shadow-2xl">
        ...
        <div className="p-6 space-y-8">
          {/* Wallet connect toggle — independent from token economy */}
          <div className="flex items-center justify-between rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
            <div className="pr-4">
              ...
              <p className="text-xs text-white/40 mt-1">Show a Connect Wallet button to players. Required for any token / payment flow.</p>
            </div>
            <button
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-white/80">Enable Token Economy</p>
              <p className="text-xs text-white/30">Launch an SPL token on Solana tied to your game</p>
            </div>
            <button
              onClick={() => update('enabled', !config.enabled)}
              disabled={!config.walletConnect.enabled}
              title={!config.walletConnect.enabled ? 'Enable wallet connection first' : undefined}
              className={`w-11 h-6 rounded-full transition-colors ${config.enabled ? 'bg-[#8b7ec8]' : 'bg-white/10'} disabled:opacity-40 disabled:cursor-not-allowed`}
            >
              <div className={`w-5 h-5 rounded-full bg-white shadow transition-transform mx-0.5 ${config.enabled ? 'translate-x-5' : ''}`} />
            </button>
          </div>
          ...
          {config.enabled && (
            <>
              ...
              <div>
                <h3 className="text-xs font-semibold text-white/40 uppercase tracking-wider mb-3">Economy Model</h3>
                <div className="grid grid-cols-1 gap-2">
                  {MODELS.map((m) => (
                    <button
                      key={m.id}
                      onClick={() => update('model', m.id as GameEconomyConfig['model'])}
              ...
              {/* Launch Platform */}
              {(config.model === 'token-launch' || config.model === 'play-to-earn') && (
                <div>
              ...
              {/* Pay to Play Settings */}
              {(config.model === 'pay-to-play' || config.model === 'tournament') && (
                <div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <SliderRow
                      ...
                      value={config.rewards.winnerShare}
                      onChange={(v) => updateNested('rewards', 'winnerShare', v)}
                      color="#10b981"
                    />
                    <SliderRow
                      label="Creator (You)"
                      value={config.rewards.creatorShare}
                      onChange={(v) => updateNested('rewards', 'creatorShare', v)}
                      color="#8b7ec8"
                    />
                    <SliderRow
                      label="Platform Fee"
                      value={config.rewards.platformShare}
                      onChange={(v) => updateNested('rewards', 'platformShare', v)}
                      color="#f59e0b"
                    />
              ...
              {/* Staking removed — not implemented on-chain. The schema fields
                  remain for backwards compatibility but the UI for them is
                  hidden to avoid promising something we don't deliver. */}
              ...
              <div className="rounded-xl border border-[#8b7ec8]/20 bg-[#8b7ec8]/[0.04] p-4">
                ...
                <div className="space-y-1 text-xs text-white/40">
                  <p>Model: <span className="text-white/70">{MODELS.find(m => m.id === config.model)?.label}</span></p>
                  {config.token.symbol && <p>Token: <span className="text-white/70">${config.token.symbol}</span></p>}
                  {(config.model === 'token-launch' || config.model === 'play-to-earn') && <p>Launch: <span className="text-white/70">{LAUNCH_PLATFORMS.find(p => p.id === config.launchPlatform)?.label}</span></p>}
                  ...
                  <p>Creator Revenue: <span className="text-white/70">{config.rewards.creatorShare}%</span> of all transactions</p>
        ...
        <div className="sticky bottom-0 flex items-center justify-between gap-3 border-t border-white/[0.06] bg-[#0d0d14] px-6 py-4">
          ...
          <button
            onClick={onClose}
            className="rounded-lg bg-[#8b7ec8] px-5 py-2 text-xs font-semibold text-white hover:bg-[#7a6db8] transition-colors"
          >
            Done
          </button>
        </div>
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <input
        ...
        style={{
          background: `linear-gradient(to right, ${color} 0%, ${color} ${value}%, rgba(255,255,255,0.06) ${value}%, rgba(255,255,255,0.06) 100%)`,
        }}
      />
